...
def do_stuff():
    for type_key in data.keys():
        ...
        for (my_key, my_data) in data[type_key].items():
            ...
            try:
                # If we return Non, we will fail, if so, we won't save
...
def get_Actors(my_data):
    output = dict(name=my_data["nameEnemies"],
                  image="",
                  fullname="",
                  class_name=data["Classes"][my_data["class_id"]]["name"],
                  caption=my_data["description"].replace("\r", "").replace("\n", " "),
                  hd="",
                  AGI="", armor_types="", ATK="", Crit_Chance="", DEF="", Evasion="",
                  feature1="", feature10="", feature11="", feature12="", feature2="", feature3="", feature4="",
                  feature5="", feature6="", feature7="", feature8="", feature9="", Hit_Rate="", HP="",
                  level="", LUCK="", SATK="", SDEF="", skill_types="", skills="", SP="",
                  Target_Rate="", weapon_types="", win_quotes="", level_quotes="", drop_quotes="", auto_state="")
    if my_data["nickname"] == "":
        output["fullname"] = "%s" % my_data["name"]
        output["image"] = "%s.png" % my_data["name"]
        output["hd"] = "%s_HD.png" % my_data["nickname"]
    else:
        output["fullname"] = "%s %s" % (my_data["name"], my_data["nickname"])
        output["image"] = "%s%s.png" % (my_data["name"], my_data["nickname"])
        output["hd"] = "%s_%s_HD.png" % ((my_data["name"], my_data["nickname"]))


    #GET ARMOR WEAPON TYPES and EVA, TRG etc
    feature_no = 1
    #GET DATA FROM ACTOR---------------------------------------------------------------
        ...
        if single_feature["code"] == 52:
            ...
            if value not in output["armor_types"]:
                if output["armor_types"] != "":
                    output["armor_types"] += " / "
                output["armor_types"] += "[[%s]]" % value
        #SKILLS
        elif single_feature["code"] == 41:
            value = extradata["ActorSkills"][str(single_feature["data_id"])]
            if value not in output["skill_types"]:
                if output["skill_types"] != "":
                    output["skill_types"] += " / "
                output["skill_types"] += "[[%s]]" % value
        #Weapon
        elif single_feature["code"] == 51:
            value = extradata["WeaponType"][str(single_feature["data_id"])]
            if value not in output["skill_types"]:
                if output["weapon_types"] != "":
                    output["weapon_types"] += " / "
                output["weapon_types"] += "[[%s]]" % extradata["WeaponType"][str(single_feature["data_id"])]
        #EX PARAMATER
    ...
    learn_count = 1
    #SKILL LEARNING
    for learning in data["Classes"][my_data["class_id"]]["learnings"]:
        level = learning["level"]
        description = data["Skills"][learning["skill_id"]]["description"].replace("\r", "").replace("\n", " ")
        name = "[[%s|%s]]" % (get_filename_of_item(data["Skills"][learning["skill_id"]]["name"], learning["skill_id"], "Skills"), data["Skills"][learning["skill_id"]]["name"])
        cost = data["Skills"][learning["skill_id"]]["mp_cost"] if data["Skills"][learning["skill_id"]]["mp_cost"] != 0 \
            else data["Skills"][learning["skill_id"]]["tp_cost"]

        if learn_count == len(data["Classes"][my_data["class_id"]]["learnings"]):
            output["skills"] += '|-\n| class="Content Text BottomLeft" |%s\n| class="Content Text" |%s\n| class="Content" |%d\n| class="Content BottomRight" |%d\n' %\
                (name,
                 description,
                 level,
                 cost)
    ...
    params = data["Classes"][1]["params"]["data"][1].split(" ")

    #PARAMATERS
    output["HP"] = int(params[0], 16)
    output["SP"] = int(params[1], 16)
    output["ATK"] = int(params[2], 16)
    output["DEF"] = int(params[3], 16)
    output["SATK"] = int(params[4], 16)
    output["SDEF"] = int(params[5], 16)
    output["AGI"] = int(params[6], 16)
    output["LUCK"] = int(params[7], 16)
    output["level"] = my_data["initial_level"]

    initial_note_text = my_data["note"]

    if "auto state:" in initial_note_text:
        state_id = re.sub(r"<auto state: (\d+)>.*", r'\1', initial_note_text.split('\n')[0])
        initial_note_text = ''.join(initial_note_text.split('\n')[1:])
        output["auto_state"] = "[[%s]]" % data["States"][int(state_id)]["name"]

    #Remove spaces in tags e.g. <win quotes> -> <win_quotes>
    notes = re.sub(r"<(\/*\w+)+\s(\w+)>", r'<\1_\2>', initial_note_text)
    #Remove \\elb
    notes = re.sub(r"\\elb\s", r'', notes)
    #Remove [new quote]
    notes = re.sub(r"\[new quote\]\r\n", r'', notes)
    #Add a root
    notes = "<note>" + notes + "</note>"
    #Parse it
    notes_xml = minidom.parseString(notes)

    for drops_quote in notes_xml.getElementsByTagName('drops_quotes')[0].firstChild.nodeValue.strip().split('\n'):
        output["drop_quotes"] += "* " + drops_quote + "\n"
    for win_quote in notes_xml.getElementsByTagName('win_quotes')[0].firstChild.nodeValue.strip().split('\n'):
        output["win_quotes"] += "* " + win_quote + "\n"
    for level_quote in notes_xml.getElementsByTagName('level_quotes')[0].firstChild.nodeValue.strip().split('\n'):
        output["level_quotes"] += "* " + level_quote + "\n"
...
def get_Weapons(my_data):
    #if get_att(my_data, "id") < 235 and isJoyful:
    #    return None
...
def get_Skills(my_data):
    if get_att(my_data, "id") < 850 and isJoyful:
        return None
    ...
    if my_data["tp_gain"] != 0:
        output["tp_gain"] = my_data["tp_gain"]

    if my_data["damage"]["type"] != 0:
        output["damage_type"] = extradata["DamageType"][str(my_data["damage"]["type"])]
        output["element_type"] = extradata["ElementType"][str(my_data["damage"]["element_id"])]
        output["formula"] = my_data["damage"]["formula"]
        output["variance"] = my_data["damage"]["variance"]
        output["critical"] = my_data["damage"]["critical"]
    ...
    for effect in my_data["effects"]:
        ...
        if effect["code"] == 11 or effect["code"] == 12:
            ...
            amount = str(int(effect[("value" + str(value_id))] * 100)) if effect["value2"] == 0 else str(
                int(effect[("value" + str(value_id))]))
            text = extradata["Effect"][str(effect["code"])][str(value_id)]
            output["effect" + str(effect_no)] = text % amount
            effect_no += 1
        # TP Recovery
        elif effect["code"] == 13:
            text = extradata["Effect"][str(effect["code"])]
        ...
        # States + Buffs
            ...
            if effect["data_id"] == 0:
                ...
                state = data["States"][effect["data_id"]]["name"]
            output["effect" + str(effect_no)] =  (text % (chance, state))
            effect_no += 1
        elif effect["code"] == 22 or effect["code"] == 33 or effect["code"] == 34:
            text = extradata["Effect"][str(effect["code"])]
            state = data["States"][effect["data_id"]]["name"]
            output["effect" + str(effect_no)] = (text % (state))
            effect_no += 1
        elif effect["code"] == 31 or effect["code"] == 32:
            text = extradata["Effect"][str(effect["code"])]
            state = data["States"][effect["data_id"]]["name"]
            turns = str(int((effect["value1"])))
            output["effect" + str(effect_no)] = (text % (state, turns))
            effect_no += 1
        elif effect["code"] == 41:
            text = extradata["Effect"][str(effect["code"])]
            output["effect" + str(effect_no)] = (text % "Escape")
            effect_no += 1
        elif effect["code"] == 42:
            text = extradata["Effect"][str(effect["code"])]
            stat = extradata["BasicStatus"][str(effect["data_id"])]
            amount = str(int(effect["value1"]))
            output["effect" + str(effect_no)] = (text % (stat, amount))
            effect_no += 1
        elif effect["code"] == 43:
            text = extradata["Effect"][str(effect["code"])]
            skill = data["Skills"][effect["data_id"]]["name"]
            output["effect" + str(effect_no)] = (text % skill)
            effect_no += 1
        elif effect["code"] == 44:
            text = extradata["Effect"][str(effect["code"])]
            event_text = data["CommonEvents"][effect["data_id"]]["name"]
            output["effect" + str(effect_no)] =  (text % event_text)
            effect_no += 1

    for x in range(effect_no, 13):
        output["effect" + str(x)] = ""

    if my_data["message2"] == "" or my_data["message2"].isspace():
        output["message"] = "(User) " + my_data["message1"]
    elif my_data["message1"] == "" or my_data["message1"].isspace():
        output["message"] = my_data["message2"]

    if my_data["required_wtype_id1"] > 1:
        output["required_weapon1"] = extradata["WeaponType"][str(my_data["required_wtype_id1"])]

    if my_data["required_wtype_id2"] > 1:
        output["required_weapon2"] = extradata["WeaponType"][str(my_data["required_wtype_id2"])]
...
def get_Enemies(my_data):
    ...
    if get_att(my_data, "id") < 314 and isJoyful:
        return None
